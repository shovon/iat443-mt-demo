...
void ofApp::setup(){
    ...
    analysisPixels.allocate(scaledWidth, scaledHeight, OF_PIXELS_RGB);
    analysisTexture.allocate(analysisPixels);
}
...
void ofApp::update(){
    ...
    int newWidth = ((float) video.getWidth()) * scaleFactor;
    int newHeight = ((float) video.getHeight()) * scaleFactor;
    ...
    const int thresh = 25; // <--- feel free to change this
...
void ofApp::draw(){
    ...
    video.draw(0, 0);
    analysisTexture.draw(0, video.getHeight());
